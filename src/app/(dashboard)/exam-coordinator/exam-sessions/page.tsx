...
} from "@/features/exam-sessions/hooks/use-exam-session-mutations";
...
const ExamSessionsPage = () => {
  ...
  const [deletingSession, setDeletingSession] = useState<ExamSession | null>(
    null
  );
  ...
  const { data, isLoading } = useExamSessionsQuery({ page, limit });
  ...
  const columns = getExamSessionColumns({
    onEdit: (session) =>
      router.push(`/exam-coordinator/exam-sessions/${session._id}/edit`),
    onDelete: (session) => setDeletingSession(session),
    onView: (session) =>
      router.push(`/exam-coordinator/exam-sessions/${session._id}`),
    ...
    <RoleGuard allowedRoles={[USER_ROLES.EXAM_COORDINATOR, USER_ROLES.ADMIN]}>
      <div className='space-y-6'>
        <div className='flex items-center justify-between'>
          ...
          <Button
            onClick={() =>
              router.push("/exam-coordinator/exam-sessions/create")
            }
          >
          ...
          <AlertDialogContent>
            <AlertDialogHeader>
              ...
              <AlertDialogDescription>
                ...
                <strong>{deletingSession?.examTitle}</strong>. This action
                cannot be undone.
              </AlertDialogDescription>
            </AlertDialogHeader>
            <AlertDialogFooter>
        ...
        <Dialog
          ...
          onOpenChange={() => setCancellingSession(null)}
        >
          <DialogContent>
            <DialogHeader>
              <DialogTitle>Cancel Exam Session</DialogTitle>
              <DialogDescription>
                You are about to cancel the exam session{" "}
                <strong>{cancellingSession?.examTitle}</strong>. Please provide
